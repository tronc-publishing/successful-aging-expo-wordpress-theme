<?php
	...
	function add_events_cta_mbox(){
		add_meta_box(
			'designbyte-events-cta',                 // Unique ID
			'Add Market CTA (button)',               // Title
			'designbyte_events_cta_markup',	         // Callback function
			'events-page',                           // (post, page, dashboard, link, attachment, custom_post_type)
			'normal',                                // location (normal, advanced, side)
			'high',                        		     // priority (high, core, default, low)
			 null								     // callback_args
		);
	} 
	add_action('add_meta_boxes', 'add_events_cta_mbox');


    function designbyte_events_cta_markup() {
        global $post;
        echo '<input type="hidden" name="events-cta-mbox-nonce" id="events-cta-mbox-nonce" value="' . wp_create_nonce( plugin_basename(__FILE__) ) . '" />';
    ...
    function designbyte_events_cta_save($post_id, $post) {
        if(!wp_verify_nonce($_POST['events-cta-mbox-nonce'], plugin_basename(__FILE__))){
            return $post->ID;
        }
        if(!current_user_can('edit_post', $post->ID))
            return $post->ID;

        $events_cta_custom_post_meta['acc-show'] = $_POST['acc-show'];
        $events_cta_custom_post_meta['acc-label'] = $_POST['acc-label'];
        $events_cta_custom_post_meta['acc-url'] = $_POST['acc-url'];

        foreach($events_cta_custom_post_meta as $key => $value) {
    ...
    add_action('save_post', 'designbyte_events_cta_save', 1, 2);
